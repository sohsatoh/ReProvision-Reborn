...
- (void)layoutSubviews {
    ...
    if (!self.noApplicationsInThisSection) {
        ...
        CGFloat yAdjust = self.contentView.frame.origin.y;
        self.icon.frame = CGRectMake(0, 0 - yAdjust, ((self.contentView.frame.size.height + yAdjust*2) / 240.0) * 400, ((self.contentView.frame.size.height + yAdjust*2) / 240.0) * 240.0);
        
        CGFloat displayNameHeight = 30;
        ...
        CGFloat bundleIdentifierHeight = 24;
...
- (void)_setupUIIfNecessary {
    ...
    if (!self.displayNameLabel) {
        ...
        self.displayNameLabel.font = [UIFont systemFontOfSize:28 weight:UIFontWeightRegular];
    ...
    if (!self.bundleIdentifierLabel) {
        ...
        self.bundleIdentifierLabel.font = [UIFont systemFontOfSize:24 weight:UIFontWeightRegular];
    ...
    if (!self.timeRemainingLabel) {
        ...
        self.timeRemainingLabel.font = [UIFont systemFontOfSize:24 weight:UIFontWeightMedium];
...
- (void)configureWithApplication:(RPVApplication*)application fallbackDisplayName:(NSString*)fallback andExpiryDate:(NSDate*)date {
    ...
    if (!application) {
        ...
        self.displayNameLabel.font = [UIFont systemFontOfSize:30 weight:UIFontWeightRegular];
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            UIImage *icon = [application tvOSApplicationIcon];
